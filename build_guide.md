...
There will be two circular holes right next to each other on the side of the printer. The black rubber cover for the power jack is optional. The power jack will need to be secured with a nut, while the power switch will simply lock into place once pushed in.  (check which part letter)
...
### Board
Install the board by the power switch using 3 (insert screw type)
...
Next fit another nut into the (component name) and line up with the hole on the (component name).
...
### Y Gantry
Insert a (nut) into the still exposed slot on the (component)
...
It should line up with the hotend fan like so, attach with (screws).
